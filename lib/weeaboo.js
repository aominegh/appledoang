...
const anime = (title) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://api.vhtear.com/kusosearch?query=${title}&apikey=${config.vhtear}`)
 ...
 */
 const neko = () => new Promise((resolve, reject) => {
    console.log('Get random picture neko...')
    fetchText('https://api.lolhuman.xyz/api/random/neko?apikey=aominehg')
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
