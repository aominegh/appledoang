...
const randomLewd = () => new Promise((resolve, reject) => {
    const tag = ['ecchi', 'lewdanimegirls', 'hentai', 'hentaifemdom', 'animearmpits', 'animefeets', 'animebooty', 'biganimetiddies', 'sideoppai', 'ahegao']
...
const nhdl = (query) => new Promise((resolve, reject) => {
    console.log('Searching Code Nhentai. . .')
    fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${query}?apikey=aominehg`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})

const nhentai = (query) => new Promise((resolve, reject) => {
    console.log('Searching Code Nhentai. . .')
    fetchJson(`https://api.lolhuman.xyz/api/nhentai/${query}?apikey=aominehg`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
