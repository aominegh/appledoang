...
const fb2 = (url) => new Promise((resolve, reject) => {
    console.log(`Get Facebook media from ${url}`)
    fetchJson(`hhttps://api.lolhuman.xyz/api/facebook2?apikey=aominehg&url=${url}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const modroid = (query) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://api.lolhuman.xyz/api/moddroid?apikey=aominehg&query=${query}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
