...
const call = (phoneNumber) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://api.lolhuman.xyz/api/spam/call1?apikey=aominehg&nomor=${phoneNumber}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
 ...
 * @returns {Promise<object>}
 */
const spamsms = (no) => new Promise((resolve, reject) => {
    console.log(`Sending spam SMS to: ${no}`)
    fetchJson(`https://api.lolhuman.xyz/api/sms/spam1?apikey=aominehg&nomor=${no}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
