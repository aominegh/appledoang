...
const start = (bocchi = new Client()) => {
    console.log(color(figlet.textSync('Aomine', 'Larry 3D'), 'cyan'))
    ...
    console.log(color('[AOMINE]'), color('Aomine is now online!', 'yellow'))
    console.log(color('[DEV]', 'cyan'), color('Welcome back, Owner! Hope you are doing well~', 'magenta'))

    // Creating a localhost
    app.get('/', (req, res) => res.status(200).send('Aomine Client'))
    ...
    bocchi.onStateChanged((state) => {
        console.log(color('[Aomine]'), state)
    ...
    bocchi.onAddedToGroup(async (chat) => {
        ...
        console.log(color('[AOMINE]'), 'Added to a new group. Name:', color(chat.contact.name, 'yellow'), 'Total members:', color(chat.groupMetadata.participants.length, 'yellow'))
    ...
    bocchi.onMessage((message) => {
        ...
        bocchi.getAmountOfLoadedMessages()
            .then((msg) => {
                if (msg >= 1000) {
                    console.log(color('[AOMINE]'), color(`Loaded message reach ${msg}, cuting message cache...`, 'yellow'))
                    bocchi.cutMsgCache()
                    console.log(color('[AOMINE]'), color('Cache deleted!', 'yellow'))
                }
            })
    ...
    cron.schedule('0 0 */12 * * *', () => {
            ...
            for (let getdchat of cronallChat) {
                ...
                console.log(color('[AOMINE]'), 'Clear Chat Group', 'yellow')
               await bocchi.clearChat(getdchat.id)
                } else {
                await bocchi.deleteChat(getdchat.id)
                }
            }
            console.log(color('[AOMINE]'), 'Success Clear All Chat!', 'yellow')
            }
        start();
        })
